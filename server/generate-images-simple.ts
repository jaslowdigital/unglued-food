...
async function generateImagesSimple() {
  // Process only first 10 recipes for now
  const recipesToProcess = glutenFreeRecipes100.slice(0, 10);
  const generatedImages: Record<string, string> = {};
  
  console.log(`Generating images for ${recipesToProcess.length} recipes...\n`);
